...
Let $\mathcal{N}$ be a neighborhood function defined on a CC $\mathcal{X}$. Moreover, let $\mathcal{Y}=\{y_1,\ldots,y_n\}$ and $\mathcal{Z}=\{z_1,\ldots,z_m\}$ be two collections of cells in $\mathcal{X}$ such that $\mathcal{N}(y_{i}) \subseteq \mathcal{Z}$ for all $1\leq i \leq n$. The \textbf{neighborhood matrix of $\mathcal{N}$ with respect to $\mathcal{Y}$ and $\mathcal{Z}$} is the $|\mathcal{Z}| \times |\mathcal{Y}|$ binary matrix $G$ whose $(i,j)$-th entry $[G]_{ij}$ has value $1$ if $z_i \in \mathcal{N}(y_j)$ and $0$ otherwise.
```
...
Let $(S,\mathcal{X}, \mbox{rk})$ be a CC. Two cells $x, y\in\mathcal{X}$ of the CC are called \textbf{incident} if either $x \subsetneq y$ or $y \subsetneq x$. In particular, the \textbf{down-incidence neighborhood function} $\mathcal{N}_{\searrow}(x)$ of a cell $x\in\mathcal{X}$ is defined to be the set $\{ y\in \mathcal{X} \mid y \subsetneq x\}$, while the \textbf{up-incidence neighborhood function} $\mathcal{N}_{\nearrow}(x)$ of $x$ is defined to be the set $\{ y\in \mathcal{X} \mid x \subsetneq y\}$.
```
...
Let $(S,\mathcal{X}, \mbox{rk})$ be a CC. For any $k\in\mathbb{N}$, the \textbf{$k$-down incidence neighborhood function} $\mathcal{N}_{\searrow,k}(x)$ of a cell $x\in\mathcal{X}$ is defined to be the set $\{ y\in \mathcal{X} \mid y \subsetneq x, \mbox{rk}(y)=\mbox{rk}(x)-k \}$. The \textbf{$k$-up incidence neighborhood function} $\mathcal{N}_{\nearrow,k}(x)$ of $x$ is defined to be the set $\{ y\in \mathcal{X} \mid y \subsetneq x, \mbox{rk}(y)=\mbox{rk}(x)+k \}$.
```
